...
DLLUS2 const char *us2_animation_get_name(source2::resource::Animation *animation) { return animation->GetName().c_str(); }
DLLUS2 float us2_animation_get_fps(source2::resource::Animation *animation) { return animation->GetFPS(); }
DLLUS2 uint32_t us2_animation_get_frame_count(source2::resource::Animation *animation) { return animation->GetFrames().size(); }
...
DLLUS2 uint32_t us2_animation_frame_get_bone_count(source2::resource::Frame *frame) { return frame->GetBones().size(); }
...
DLLUS2 void us2_animation_frame_bone_get_position(const source2::resource::FrameBone *frameBone, float *outPos) { std::memcpy(outPos, &frameBone->position, sizeof(frameBone->position)); }
DLLUS2 void us2_animation_frame_bone_get_rotation(const source2::resource::FrameBone *frameBone, float *outRot) { std::memcpy(outRot, &frameBone->rotation, sizeof(frameBone->rotation)); }
}
